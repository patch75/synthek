...
Pattern identique à `genererPuce()` dans `ia.js`.
...
### Remplacer le bloc lignes 151–168 (AVANT)
```javascript
...
## Étape 5 — `services/ia.js` — `analyserProjet()` lignes 154–161

### Remplacer le bloc (AVANT)
```javascript
...
### Par (APRÈS) — insérer juste après la ligne 152 (`const [...]  = await Promise.all(...)`)
```javascript
...
# 3. Créer extractFaits.js
# → créer backend/src/services/extractFaits.js
